...
# NumPy-only MLP + AdaMax. Колір точки визначається за схемою X->D та |D[14]-D[k]|<e
# ВАЖЛИВО: X = RMS-величина КОРЕКЦІЇ ВАГ (норма фактичного оновлення) по кожному нейрону
# Де X — величина повної корекції ваг для кожного нейрона отримана оптимізатором AdaMax
import os
import sys
import tarfile
import pickle
...
RUNS_DIR          = Path("runs_dynmaps_corr_adamax")
...
class AdaMaxState:
    def __init__(self, Ws, bs):
        ...
        self.uW = [np.zeros_like(W) for W in Ws]

        self.mb = [np.zeros_like(b) for b in bs]
        self.ub = [np.zeros_like(b) for b in bs]

        self.t = 0


def adamax_step_return_updates(
        Ws, bs, dWs, dbs, st: AdaMaxState, *,
        lr, beta1, beta2, eps=1e-8
):
    st.t += 1
    updW, updb = [], []
    ...
    for i in range(len(Ws)):
        ...
        st.mW[i] = beta1 * st.mW[i] + (1.0 - beta1) * gW
        st.mb[i] = beta1 * st.mb[i] + (1.0 - beta1) * gb

        st.uW[i] = np.maximum(beta2 * st.uW[i], np.abs(gW))
        st.ub[i] = np.maximum(beta2 * st.ub[i], np.abs(gb))

        mW_hat = st.mW[i] / (1.0 - beta1 ** st.t)
        mb_hat = st.mb[i] / (1.0 - beta1 ** st.t)

        stepW = lr * (mW_hat / (st.uW[i] + eps))
        stepb = lr * (mb_hat / (st.ub[i] + eps))
...
def train_and_get_X_weight_corrections(
    ...
    for ep in range(1, epochs + 1):
        ...
        for i in range(0, N, batch_size):
            ...
            updW_adaptive, _ = adamax_step_return_updates(
                Ws, bs, dWs, dbs, opt,
                lr=alpha, beta1=beta1, beta2=beta2
            )

            actual_upd_layer = updW_adaptive[layer_idx]

            col_norms = np.linalg.norm(actual_upd_layer, axis=0)
...
def main():
    ...
    for cls in range(10):
        ...
        plt.title(f"Dynamic regimes map by weight corrections (AdaMax, class {cls})\nalpha={ALPHA}, epochs/pt={EPOCHS_PER_POINT}")
        plt.xlabel(r'$\beta_2$'); plt.ylabel(r'$\beta_1$')
        plt.tight_layout()
        ts = timestamp()
        out_png = RUNS_DIR / f"map_adamax_corr_b1_vs_b2_class{cls}_a{ALPHA}_{ts}.png"
        plt.savefig(out_png, dpi=150); plt.close()
        print(f"[SAVED] {out_png}")

        out_csv = RUNS_DIR / f"map_adamax_corr_b1_vs_b2_class{cls}_a{ALPHA}_{ts}.csv"
