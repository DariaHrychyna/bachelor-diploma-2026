...
# NumPy MLP + AdamW (decoupled weight decay).
# Колір точки: X -> D[i]=X[i+1]^2+X[i]^2 і |D[14]-D[k]|<e.
# X = RMS-величина ФАКТИЧНОЇ КОРЕКЦІЇ ВАГ (норма кроку оновлення) по кожному нейрону вибраного прихованого шару.
...
EPOCHS_PER_POINT  = 40          # епох на одну точку карти
ALPHA             = 0.01        # learning rate
WEIGHT_DECAY      = 0.01       # λ у AdamW (decoupled L2)
INCLUDE_WD_IN_X   = True       # якщо True — X враховує внесок weight decay у фактичному кроці
...
RUNS_DIR          = Path("runs_cifar100_dynmaps_corr_adamw")
...
def load_cifar100_numpy():
    ...
    CIFAR-100 (python version): файли 'train'/'test' з ключами 'data','fine_labels',...
    """
    download_cifar100()
    with tarfile.open(CIFAR_TAR, "r:gz") as tar:
        ...
        data = batch["data"]  # (50000, 3072) R(1024),G(1024),B(1024)
...
def adamw_step_return_updates(
    ...
    """
    Крок AdamW із декупльованим weight decay.
    Повертає:
      - updW[i]: фактична корекція ваг (що віднято від W), включно з -lr*λ*W якщо include_wd_in_return=True
    ...
    for i in range(len(Ws)):
        ...
        mW_hat = st.mW[i] / (1.0 - beta1 ** st.t) if beta1 < 1.0 else st.mW[i]
        vW_hat = st.vW[i] / (1.0 - beta2 ** st.t) if beta2 < 1.0 else st.vW[i]

        adaptive_step = lr * (mW_hat / (np.sqrt(vW_hat) + eps))
        wd_step       = lr * weight_decay * Ws[i]  # decoupled

        stepW_to_apply = adaptive_step
        if weight_decay != 0.0:
            stepW_to_apply = adaptive_step
        Ws[i] -= adaptive_step
        if weight_decay != 0.0:
        ...
        stepW_return = adaptive_step + (wd_step if include_wd_in_return else 0.0)
        updW.append(stepW_return.copy())

        st.mb[i] = beta1 * st.mb[i] + (1.0 - beta1) * dbs[i]
        st.vb[i] = beta2 * st.vb[i] + (1.0 - beta2) * (dbs[i] ** 2)

        mb_hat = st.mb[i] / (1.0 - beta1 ** st.t) if beta1 < 1.0 else st.mb[i]
        vb_hat = st.vb[i] / (1.0 - beta2 ** st.t) if beta2 < 1.0 else st.vb[i]
...
def main():
    ...
    for cls in CLASSES_TO_RUN:
        ...
        plt.title(f"CIFAR-100 dynamic regimes (AdamW) by weight corrections (class {cls})\n"
                  f"alpha={ALPHA}, wd={WEIGHT_DECAY}, epochs/pt={EPOCHS_PER_POINT}")
        plt.xlabel(r'$\beta_2$'); plt.ylabel(r'$\beta_1$')
        plt.tight_layout()
        ts = timestamp()
        out_png = RUNS_DIR / f"c100_adamw_map_corr_b1_vs_b2_class{cls}_a{ALPHA}_wd{WEIGHT_DECAY}_{ts}.png"
        plt.savefig(out_png, dpi=150); plt.close()
        print(f"[SAVED] {out_png}")

        out_csv = RUNS_DIR / f"c100_adamw_map_corr_b1_vs_b2_class{cls}_a{ALPHA}_wd{WEIGHT_DECAY}_{ts}.csv"
